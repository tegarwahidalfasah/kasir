...
export function runSeed({ force = false, quiet = false } = {}) {
  ...
  for (const f of FIN) {
    ...
    f.bom.forEach(([key, qty, waste], i) => {
      exec(`INSERT INTO item_recipes (id, parent_id, raw_item_id, qty, unit, waste_pct, is_optional, sort_order) VALUES
            (?,?,?,?,?, ?,?,?)`, uid('rcp'), id, rawId[key], qty, RAW.find((r) => r.key === key).unit, waste, 0, i);
    });
    if (f.addons === 'shot') {
  ...
  generateAlerts(storeId, { days: 30, lookaheadDays: 14 });
  log(`✅ Seed selesai: ${created} transaksi, ${RAW.length} bahan baku, ${FIN.length} barang jadi`);
  if (quiet) console.log('KASIR_READY');
  log('');
  log('   Login demo (semua password: rahasia123)');
  log('   ┌──────────┬────────────────────────┐');
  log('   │ budi     │ Pemilik (akses penuh)  │');
  log('   │ rina     │ Manajer                │');
  log('   │ sari     │ Manajer Inventaris     │');
  log('   │ dewi/adi │ Kasir                  │');
  log('   └──────────┴────────────────────────┘');
  log('   PIN kasir: dewi=4444, adi=5555');

  return { ok: true, created, rawCount: RAW.length, finCount: FIN.length };
}
...
if (isDirectRun) {
  ...
  runSeed({ force: FORCE, quiet: QUIET });
